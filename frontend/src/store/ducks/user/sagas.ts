...
export function* startUseStreamNetRequest() {
    try {
        yield put(setUserLoadingStatus(LoadingStatus.LOADING));
        const response: AxiosResponse<boolean> = yield call(UserApi.startUseStreamNet);
...
export function* userSaga() {
    ...
    yield takeLatest(UserActionsType.START_USE_STREAMNET, startUseStreamNetRequest);
